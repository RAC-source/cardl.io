-- cardl.io Database Schema (KORRIGIERT für Apple Sign-In)
...
-- EINFACHE Policy: Alle authentifizierten Benutzer können alle Profile lesen
-- (Für Dashboard-Zwecke - kann später eingeschränkt werden)
CREATE POLICY "Authenticated users can view all profiles" ON user_profiles
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- Funktion zum Erstellen eines Benutzerprofils (KORRIGIERT - ohne RLS-Beschränkungen)
...
BEGIN
  ...
  INSERT INTO user_profiles (
    ...
    COALESCE(NEW.raw_app_meta_data->>'provider', 'apple'),
